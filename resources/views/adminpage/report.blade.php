                                ...
                                <td class="p-3">
                                    <div class="flex justify-center">
                                        <button
                                            ...
                                            onclick="showTransactionModal(this)" data-id="{{ $sl->id }}"
                                            data-user="{{ $sl->user->name ?? '-' }}"
                                            data-date="{{ \Carbon\Carbon::parse($sl->sale_date)->format('d/m/Y') }}"
                                            data-customer="{{ $sl->customer->name ?? '-' }}" data-items='@json($sl->saleItems->map(fn($item) => [
                                            "name" => $item->product->name ?? ' Unknown', "qty"=> $item->quantity
                                            ]))'
                                            data-payment="{{ $sl->payments->first()->payment_method ?? '-' }}"
                                            data-type="{{ ucfirst(str_replace('_', ' ', $sl->type)) }}"
                                            data-amount="Rp{{ number_format($sl->total, 0, ',', '.') }}">
                                        ...
                                        <button
                                            ...
                                            onclick="setDeleteTransaction({{ $sl->id }})">
                                            <span class="material-symbols-outlined">
                                                cancel
                                            </span>
                                        </button>
