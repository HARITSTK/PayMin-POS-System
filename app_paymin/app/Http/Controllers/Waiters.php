...
class Waiters extends BaseController
{
    ...
    public function home() {
        ...
        return view('waiterspage/home',  compact('user', 'admissionFee', 'TotalItems', 'TotalCustomers', 'TotalCustomers', 'products', 'lowStocks', 'ordersToday', 'salesGrowth'));
    }

    // ORDER
    public function order() {

        $totalOrderReady = DB::table('sales')->where('status', 'ready')->count();
        $totalOrderServed = DB::table('sales')->where('status', 'served')->count();
        // $products = Mdl_Product::with('category', 'customer')->where('status', 'proses')->get();
        $sales = Mdl_Sales::with('saleItems.sale.customer')->where('status', 'ready')->get();


        return view('waiterspage/order',compact('totalOrderReady', 'totalOrderServed', 'sales'));
    }
    ...
    public function orderupdate(Request $request)
    {
        ...
        // Update status pesanan misalnya jadi 'proceed'
        $order = Mdl_Sales::findOrFail($orderId);
        $order->status = 'served';
        $order->save();

        return redirect()->back()->with('message', 'Order has been submitted.');
    }
    ...
    public function setting() {
        ...
        return view('waiterspage/setting', compact('user'));
    }
